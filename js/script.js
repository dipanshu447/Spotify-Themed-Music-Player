...
let displayAlbums = async () => {
    let fetchAlbum = await fetch(`http://127.0.0.1:3000/albums/`);
    ...
    for (let i = 0; i < a.length; i++) {
        ...
        if (e.href.includes('/albums')) {
            ...
            // get the meta deta
            let fetchAlbum = await fetch(`http://127.0.0.1:3000/albums/${folder}/info.json`);
            ...
            cardContainer.innerHTML = cardContainer.innerHTML + `<div data-folder="${folder}" class="card">
                        ...
                        <img src="/albums/${folder}/cover.jpg"
                            alt="album-photo">
    ...
    Array.from(document.getElementsByClassName("card")).forEach(e => {
        e.addEventListener("click", async item => {
            albums = await getSongs(`albums/${item.currentTarget.dataset.folder}`);
            playmusic(albums[0].split(`/${currentFolder}/`)[1].replaceAll("%20", " "));
        })
    })
}
...
(async () => {
    ...
    await getSongs("albums/deeep-vibes");
    playmusic(albums[0].split(`/${currentFolder}/`)[1].replaceAll("%20", " "), true);
    ...
    document.getElementById('next').addEventListener("click", () => {
        let index = albums.indexOf(`http://127.0.0.1:3000/${currentFolder}/` + currentSong.src.split('/').slice(-1)[0]);

        if ((index + 1) <= albums.length - 1) {
            playmusic(albums[index + 1].split(`/${currentFolder}/`)[1]);
        }
    })
